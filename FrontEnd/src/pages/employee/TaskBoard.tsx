...
import { MapPin, CheckCircle2, CheckCheck, Loader2 } from "lucide-react";
...
const TaskBoard: React.FC = () => {
  ...
  // Moved fetchTasks outside of useEffect so it can be called globally within the component
  const fetchTasks = useCallback(async () => {
    try {
      ...
      // Fetch all works, then filter for the logged-in employee
      const data = await api.get("/works");
      const myTasks = data.filter((t: Task) => t.AssigneeName === user.Name);
      setTasks(myTasks);
    } catch (err) {
  ...
  const handleQuickComplete = async (task: Task) => {
    try {
      ...
      await api.put(`/works/${task.WorkItemID}/progress`, {
        status: "Completed",
        employeeId,
        contextNote: "Task completed from Task Board",
      });
      fetchTasks();
    } catch (err) {
        ...
        <div className="flex-1 flex gap-6 overflow-x-auto pb-4 custom-scrollbar">
          ...
          <div className="flex flex-col min-w-[340px] max-w-[360px] w-full bg-slate-100/80 backdrop-blur-sm rounded-2xl border border-slate-200/60 p-4 gap-4 h-max shadow-sm">
            <div className="flex justify-between items-center px-2 pb-1">
              <h2 className="text-zinc-900 text-xl font-bold font-['Inter']">
                Pending
              </h2>
            ...
            {pendingTasks.map((task) => (
              ...
              >
                <div className="flex justify-between items-center">
                  <span className="text-slate-500 text-xs font-bold font-['Inter']">
                    T-{task.WorkItemID}
                  </span>
                  {task.Priority === "High" && (
                    <div className="flex items-center gap-1.5 px-2 py-1 bg-red-50 rounded-md">
                      <div className="w-2 h-2 rounded-full bg-red-600"></div>
                      <span className="text-red-700 text-xs font-bold font-['Inter'] uppercase tracking-wider">
                        Urgent
                      </span>
                    </div>
                  )}
                </div>
                <h3 className="text-zinc-900 text-base font-bold font-['Inter'] group-hover:text-sky-700 transition-colors">
                  {task.Title}
                </h3>
                <div className="flex items-center gap-2 text-slate-500">
                  <MapPin size={14} />
                  <span className="text-xs font-semibold font-['Inter'] truncate">
                    {task.Description}
                  </span>
                </div>
                <div className="flex items-center gap-2 mt-2">
                  ...
                  <button
                    onClick={() =>
                      setActiveTaskForUpdate({
                        id: `T-${task.WorkItemID}`,
                        name: task.Title,
                        status: task.Status || "Pending",
                      })
                    }
                    className="flex-1 py-2.5 bg-white border border-slate-300 text-slate-700 hover:bg-slate-50 transition-colors rounded-lg text-sm font-semibold font-['Inter'] active:scale-[0.98]"
                  >
                    Update
                  </button>
                </div>
              ...
              >
                ...
                <div className="flex items-center gap-2 mt-3">
                  <button
                    ...
                    className="flex-1 py-2.5 bg-emerald-600 hover:bg-emerald-700 transition-colors text-white rounded-lg text-sm font-semibold font-['Inter'] shadow-sm active:scale-[0.98]"
                  >
                    Complete
                  </button>
                  <button
                    ...
                    className="flex-[0.6] py-2.5 bg-white border border-slate-300 text-slate-700 hover:bg-slate-50 transition-colors rounded-lg text-sm font-semibold font-['Inter'] active:scale-[0.98]"
                  >
                    Update
                  </button>
                  <button
                    onClick={() => setHandoverWorkId(task.WorkItemID)}
                    className="flex-[0.6] py-2.5 bg-white border border-amber-300 text-amber-700 hover:bg-amber-50 transition-colors rounded-lg text-sm font-semibold font-['Inter'] active:scale-[0.98]"
                  >
                    Handover
                  </button>
                </div>
          ...
          <div className="flex flex-col min-w-[340px] max-w-[360px] w-full bg-slate-100/40 rounded-2xl border border-slate-200/50 p-4 gap-4 h-max opacity-80 hover:opacity-100 transition-opacity">
            <div className="flex justify-between items-center px-2 pb-1">
              <h2 className="text-slate-600 text-xl font-bold font-['Inter']">
                Completed
              </h2>
            ...
            {completedTasks.map((task) => (
              ...
              >
                <div className="flex justify-between items-center">
                  <span className="text-slate-400 text-xs font-bold font-['Inter'] line-through">
                    T-{task.WorkItemID}
                  </span>
                  <CheckCircle2 size={18} className="text-emerald-600/80" />
                </div>
                <h3 className="text-slate-600 text-base font-bold font-['Inter'] mt-1">
                  {task.Title}
                </h3>
