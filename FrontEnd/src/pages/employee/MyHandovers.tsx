...
const MyHandovers: React.FC = () => {
  ...
  const [activeFilter, setActiveFilter] = useState<'all' | 'sent' | 'received'>('received');
                    ...
                    return (
                      <React.Fragment key={ho.HandOverID}>
                        <tr className={`hover:bg-slate-50 transition-colors ${isPendingForMe ? 'bg-amber-50/30' : ''}`}>
                          ...
                          <td className="px-6 py-4">
                            <span className={`px-2.5 py-1 text-xs font-bold rounded-md border inline-block ${ho.Status === 'Approved' ? 'bg-green-50 text-green-700 border-green-200' :
